...
def classify_force_commands(input_file, output_file):
    ...
    # Feature Engineering: Create additional labels (if needed)
    # For example, you might want to add a binary label for swiping
    data['swiping_binary'] = data['swiping in motion'].apply(lambda x: 1 if x == "swiping" else 0)

    # You can also create other feature-engineered columns based on your analysis needs
    # Example: Adding a label based on a combination of forces (hypothetical example)
    def combined_force_label(row):
        if row['x force command'] > 15 and row['z force command'] < -4:
            return "High x, Low z"
        return "Normal"

    data['combined force label'] = data.apply(combined_force_label, axis=1)
    ...
    output_data = data[['x force command', 'y force command', 'z force command', 
                        ...
                        'swiping in motion', 'swiping_binary', 'combined force label']]
